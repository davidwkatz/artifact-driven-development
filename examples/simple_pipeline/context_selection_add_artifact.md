...
This is a hypothetical pre-change example: assume the current pipeline does not yet include `deduplicated_events`.
...
For this question, the LLM should receive:

the question
the relevant entries from artifacts.yaml
the current dependency chain
a brief description of each artifact
optionally, small samples from the CSV files
Selection reasoning
